...
#pragma scop
    for (t = 0; t < T; t++) {
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
                A[1][i][j] = (exp(-0.001*t)/4)*((i-1) < 0 ? A[0][N-1][j]: A[0][i - 1][j]) + (exp(-0.001*t)/4)*((j+1) >= N ? A[0][i][0] : A[0][i][j + 1])
                                    + (exp(-0.001*t)/4)*((i+1) >= N ? A[0][0][j] : A[0][i + 1][j]) + (exp(-0.001*t)/4)*((j-1) < 0 ? A[0][i][N-1] : A[0][i][j - 1])
                                    + (1-exp(-0.001*t))*A[0][i][j];
            }
        }
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
                A[0][i][j] = (exp(-0.001*t)/4)*((i-1) < 0 ? A[1][N-1][j]: A[1][i - 1][j]) + (exp(-0.001*t)/4)*((j+1) >= N ? A[1][i][0] : A[1][i][j + 1])
                                    + (exp(-0.001*t)/4)*((i+1) >= N ? A[1][0][j] : A[1][i + 1][j]) + (exp(-0.001*t)/4)*((j-1) < 0 ? A[1][i][N-1] : A[1][i][j - 1])
