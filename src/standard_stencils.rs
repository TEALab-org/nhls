...
pub fn jacobi_2d(
    ...
    Stencil::new(
        ...
        move |args: &[f64; 9]| {
            let center = args[4];
            let weighted_sum = weight * (args[0] + args[1] + args[2] +
                                         args[3] + args[5] +          
...
pub fn jacobi_3d(
    ...
    Stencil::new(
        ...
        move |args: &[f64; 27]| {
            let center = args[13];
            let weighted_sum = weight * args.iter().enumerate().filter(|(i, _)| *i != 13)
                .map(|(_, &val)| val)
                .sum::<f64>();
            weighted_sum + (1.0 - weight) * center
        },
...
pub fn gaussian_blur_1d() -> StencilF64<impl StencilOperation<f64, 3>, 1, 3> {
    Stencil::new(
        ...
        move |args: &[f64; 3]| {
            ...
            let weighted_sum: f64 = args.iter().zip(kernel.iter()).map(|(a, k)| a * k).sum();
            weighted_sum / 4.0
        },
    )
}
    ...
    Stencil::new(
        ...
        move |args: &[f64; 27]| {
            let kernel = [
                ...
                2.0, 4.0, 2.0, 4.0, 8.0, 4.0, 2.0, 4.0, 8.0, 4.0, 
                1.0, 2.0, 1.0, 2.0, 4.0, 2.0, 1.0, 2.0, 1.0,
            ];
            let weighted_sum: f64 = args.iter().zip(kernel.iter()).map(|(a, k)| a * k).sum();
            weighted_sum / 64.0
